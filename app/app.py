...
# ===================== CONFIG BÁSICA =====================
st.set_page_config(page_title="Painel de Bônus - Starcheck (T1-2026)", layout="wide")
...
# Peso relativo de PRODUÇÃO por cidade, somando ~0.40 (40%) por supervisora
SUPERVISORAS_CIDADES = {
    "SAMMYRA JISELE BRITO REIS": {"BACABAL": 0.20, "CODÓ": 0.20},
    "GEISE ALINE MACEDO DE MEDEIROS": {"BALSAS": 0.20, "PINHEIRO": 0.20},
    "CHRISTIANE SILVA GUIMARÃES": {"SÃO LUIS": 0.20, "CAXIAS": 0.20},
    "ELIVANDY CRUZ DA SILVA": {
        "BACABAL": 0.066, "CODÓ": 0.066, "BALSAS": 0.067,
        "PINHEIRO": 0.067, "SÃO LUIS": 0.067, "CAXIAS": 0.067
    }
}
...
if filtro_mes == "TRIMESTRE":
    try:
        df_j = ler_planilha("JANEIRO")
        df_a = ler_planilha("FEVEREIRO")
        df_s = ler_planilha("MARÇO")
        st.success("✅ Planilhas carregadas: JANEIRO, FEVEREIRO e MARÇO!")
    except Exception as e:
        st.error(f"Erro ao ler a planilha: {e}")
        st.stop()

    dados_j = calcula_mes(df_j, "JANEIRO")
    dados_a = calcula_mes(df_a, "FEVEREIRO")
    dados_s = calcula_mes(df_s, "MARÇO")
